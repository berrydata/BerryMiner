...
// 	//The off
// 	constructor(address _userContract, uint _disputeFeeRequired, uint _disputePeriod, uint[] memory _requestIds, uint _granularity) UsingBerry(_userContract) public{
...
// 		address addr  = berryUserContract.berryStorageAddress();
...
// 	event BerryValuePlaced(uint _timestamp, uint _value);
// 	function getBerryValues(uint _timestamp) public returns(uint _value, bool _didGet){
...
// 		uint _initialBalance = _berry.balanceOf(address(this));
...
// 			}
// 			else{
// 				if(_berry.balanceOf(address(this)) > requestIds.length){
