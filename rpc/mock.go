...
func (c *mockClient) FilterLogs(ctx context.Context, query ethereum.FilterQuery) ([]types.Log, error) {
	tokenAbi, _ := abi.JSON(strings.NewReader(contracts1.BerryLibraryABI))
	ev := tokenAbi.Events["NonceSubmitted"]
	event1 := contracts1.BerryLibraryNonceSubmitted{
	...
	//eventResult := contracts1.BerryLibraryNonceSubmitted{}
